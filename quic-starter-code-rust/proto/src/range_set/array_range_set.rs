...
///
/// `ArrayRangeSet` uses an array representation, where each array entry
/// represents a range.
///
...
/// - Iterating over ranges should usually be faster since there is only a
///   single cache-friendly contiguous range.
///
/// `ArrayRangeSet` is especially useful for tracking ACK ranges where the
/// amount of ranges is usually very low (since ACK numbers are in consecutive
/// fashion unless reordering or packet loss occur).
