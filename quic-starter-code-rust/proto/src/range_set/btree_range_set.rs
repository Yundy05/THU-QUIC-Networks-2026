...
/// A set of u64 values optimized for long runs and random
...
impl RangeSet {
    ...
    /// Add a range to the set, returning the intersection of current ranges
    /// with the new one
...
pub struct Replace<'a> {
    set: &'a mut RangeSet,
    /// Portion of the intersection arising from a range beginning at or before
    /// the newly inserted range
    pred: Option<Range<u64>>,
    /// Union of the input range and all ranges that have been visited by the
    /// iterator so far
    range: Range<u64>,
}
...
impl Iterator for Replace<'_> {
    ...
    fn next(&mut self) -> Option<Range<u64>> {
        ...
        if next_start > self.range.end {
            // If the next successor range starts after the current range ends, there can be
            // no more overlaps. This is sound even when `self.range.end` is
            // increased because `RangeSet` is guaranteed not to contain pairs
            // of ranges that could be simplified.
            return None;
        }
        // Remove the redundant range...
        self.set.0.remove(&next_start);
        // ...and handle the case where the redundant range ends later than the new
        // range.
        let replaced_end = self.range.end.min(next_end);
        self.range.end = self.range.end.max(next_end);
        if next_start == replaced_end {
            // If the redundant range started exactly where the new range ended, there was
            // no overlap with it or any later range.
            None
        } else {
